export const personalInformation = {
	fullname: "Lê Anh Bằng",
	dob: "5th October, 2003",
	major: "Professional Communication (Bachelor), Entertainment",
	Introduction: `Hi, my name is Bang, but you can call me LAB (which is the combination of the first letters of my name), and that's also my stage name!
I graduated from Western Sydney University, Australia, majoring in Communication. I also started exploring, learning, and working in the entertainment industry as a production assistant, management assistant, and an independent artist in Vietnam from the age of 20! Up until now, I have achieved certain milestones in my personal career and contributed to the community.
`,
};

export const workExperience = [
	{
		company: "Into The Air Band",
		image: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/air_lrvp4e.png",
		roles: ["Co-founder & Manager", "Artist", "Guitarist", "Producer"],
		period: {
			start: "April 2022",
			end: "September 2024",
		},
		description:
			"Co-founded and managed the band Into The Air from April 2022 to September 2024. Also served as the Producer and Guitarist, contributing to all of the band's projects during this period.",
	},
	{
		company: "Chillies",
		image: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/chillies_dr6suj.png",
		roles: ["Assistant Manager", "Sound Technician"],
		period: {
			start: "December 2023",
			end: "Present",
		},
		description:
			"Assistant Manager and Sound Technician for the band Chillies. Responsible for ensuring that the band's performances meet their sound system requirements and performance schedules.",
	},
	{
		company: "The Flob",
		image: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967228/flob_f65125.png",
		roles: ["Assistant Manager", "Sound Technician"],
		period: {
			start: "August 2022",
			end: "October 2023",
		},
		description:
			"Served as Assistant Manager and Sound Technician for the band The Flob. Participated in producing Music Videos, Live Session, and Live Stage projects for the band.",
	},
	{
		company: "The Cassette",
		image: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/cass_eids8a.jpg",
		roles: ["Drum Technician"],
		period: {
			start: "May 2024",
			end: "Present",
		},
		description:
			"Responsible for setting up the drum tech as required by the band and managing the Backing track - Click - In-ear Monitor system during live stages.",
	},
	{
		company: "1010 Network Company Limited",
		image: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/1010_izjnjw.png",
		roles: [
			"Assistant Manager",
			"Production Assistant",
			"Sound Technician",
		],
		period: {
			start: "August 2022",
			end: "October 2023",
		},
		description:
			"Handled the role of Production Assistant for TVC, Showcase, and Music Video projects. Also served as Assistant Manager for the company's artists.",
	},
	{
		company: "TenCue Agency",
		image: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/tencue_rn16at.jpg",
		roles: ["Production Assistant"],
		period: {
			start: "June 2024",
			end: "Present",
		},
		location: "Phu Quoc, Vietnam",
		description:
			"TenCue is an agency from California, USA. In 2024, I was recruited as a Production Assistant when the company produced an event in Phu Quoc, Vietnam, guiding them to work within the Vietnamese environment.",
	},
	{
		company: "Vinama Joint Stock Company",
		image: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/vinama_ebqavx.png",
		roles: ["Internship"],
		period: {
			start: "July 2023",
			end: "October 2023",
		},
		description:
			"Interned in the Marketing Department at Vinama, a reputable company specializing in outdoor advertising and organizing prestigious advertising events in Vietnam.",
	},
];

export const awards = [
	{
		id: 1,
		event: "Band Đấu HRC",
		achievement: "Finalist",
		des: `Achieved a place in the final night of the "Band Đấu" competition (a national young band competition) Season 1, 2023, held in Hanoi, Vietnam.
`,
		images: [
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994552/3_qpjxnv.png",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994550/2_utl3gd.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994550/6_i3vdaz.png",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994549/5_bb86jf.png",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994549/4_j6wlcw.png",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994547/1_nftm6t.jpg",
		],
	},
	{
		id: 2,
		event: "Hozo Inspired Talents",
		achievement: "Semi-Final",
		des: "Reached the semifinals of the 2023 Hozo International Music Festival, a nationwide band competition held in Ho Chi Minh City, Vietnam.",
		images: [
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994602/1_y4iijz.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994600/4_bmuilm.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994598/2_eq0cps.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994597/3_rdeitp.jpg",
		],
	},
	{
		id: 3,
		event: "District Soccer Competition",
		achievement: "2 Silver Medals",
		des: "Won two silver medals in the U14 district soccer tournament in 2015 and 2016.",
		images: [
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726995016/1-min_nfz9jg.png",
		],
	},
	{
		id: 4,
		event: "District Chinese Chess Contest",
		achievement: "2 Bronze Medals",
		des: "Won two bronze medals in the district Chinese chess competition in 2013 and 2016.",
		images: [
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1726994637/1_uiwvb5.png",
		],
	},
];

export const projects = [
	{
		project: "Music Producer and Composer",
		products: [
			{
				name: "Trôi - Into The Air",
				video: "https://www.youtube.com/embed/bJtsVoGWLwg?si=-97wKwQlUe4mu_LV",
				des: "Debut project in 2023 as Music Producer and Composer for the project.",
			},
			{
				name: "Lỡ - Into The Air",
				video: "https://www.youtube.com/embed/lyPmZpyHL-M?si=y8p8X6JEpepqiIE4",
				des: "Second project with the band Into The Air, serving as Music Producer, Composer, and Production Manager for the 2023 Music Video project.",
			},
			{
				name: "Viết Cho Em - Into The Air",
				video: "https://www.youtube.com/embed/5nGtS01f1UU?si=Xn-GxQl_gyM9b6HY",
				des: "A passion project, co-produced with Producer CHIN, released in early 2024.",
			},
			{
				name: "Chạy - Into The Air",
				video: "https://www.youtube.com/embed/AzckoYoRJT0?si=_j1euJUvYkJB_1Wz",
				des: "Final project with the band Into The Air, acting as Producer in 2024.",
			},
		],
	},
	{
		project: "Production Assisstant",
		products: [
			{
				name: "Đông rồi tây - The Flob (MV)",
				video: "https://www.youtube.com/embed/Zw4Q8OjCg_E?si=7US9kRBRt9QmN24G",
			},
			{
				name: "CUỐI NGÀY (Hoành Tráng ver.) - The Flob (MV)",
				video: "https://www.youtube.com/embed/zruqus4_ENo?si=dXO1GlbP7ZzX6oam",
			},
			{
				name: "Nhạc Chơi - The Flob (MV)",
				video: "https://www.youtube.com/embed/F32Ro8RTiHU?si=-isqulJ7Pu_Sk280",
			},
			{
				name: "232 - The Flob (MV)",
				video: "https://www.youtube.com/embed/N8Y_X5wZa-E?si=039uAEN4FmixkDW6",
			},
		],
	},
	{
		project: "Event Production and Sound-Technician",
		products: [
			{
				name: "The Cassette",
				video: null,
				logo: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/cass_eids8a.jpg",
			},
			{
				name: "Chillies",
				video: "https://www.youtube.com/embed/gx_dkhoqExA?si=5lobzosAPqoOGuif",
				logo: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/chillies_dr6suj.png",
			},
			{
				name: "TenCue Agency - California",
				video: "https://www.youtube.com/embed/TxKalCK20F4?si=u9r1vM7gBjhBD2y8",
				logo: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/tencue_rn16at.jpg",
			},
			{
				name: "YES Event Agency - VietNam",
				video: "https://www.youtube.com/embed/NCseHZb5mMQ?si=8d6C6Rh-cKm9QO01",
				logo: "https://res.cloudinary.com/dbfh15hki/image/upload/v1727003266/YES_Event_jbsubz.jpg",
			},
			{
				name: "1010 Network",
				video: null,
				logo: "https://res.cloudinary.com/dbfh15hki/image/upload/v1726967227/1010_izjnjw.png",
			},
			{
				name: "The Flob",
				video: "https://www.youtube.com/embed/b8lY_lLaKQI?si=zy8qdiDEhjty8FTJ",
				logo: "https://res.cloudinary.com/dbfh15hki/image/upload/v1727004220/flob_1_wpjeg6.png",
			},
		],
	},
	{
		project: "Actor and Others",
		products: [
			{
				name: "Wish - GDucky",
				video: "https://www.youtube.com/embed/mMXdEVn_uZQ?si=yY3pkIc_AC6sadjp",
			},
			{
				name: "Vinama TVC Production",
				video: "https://www.facebook.com/plugins/video.php?height=314&href=https%3A%2F%2Fwww.facebook.com%2FCtyVinama%2Fvideos%2F155523824297844%2F&show_text=false&width=560&t=0",
			},
		],
	},
	{
		project: "Best Live Perfomances",
		products: [
			{
				name: "Lỡ - Live at Band Đấu",
				video: "https://www.youtube.com/embed/AqtvKfUPFwY?si=I_SDYD5Q8jO_1Zz7",
			},
			{
				name: "Chạy - Live at Hozo Inspired Talents",
				video: "https://www.youtube.com/embed/DrYXinwqtj4?si=AjG3L7qI4HMwH3Se",
			},
			{
				name: "Viết Cho Em - Live at Band Đấu",
				video: "https://www.youtube.com/embed/cLUQkTImsnw?si=7z1R3P7_I0y6UL-L",
			},
		],
	},
];

export const charities = [
	{
		name: "Trao cho hộ gia đình có hoàn cảnh khó khăn",
		images: [
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1727009758/IMG_5944_gckcmd.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1727009757/IMG_5943_ywd0ex.jpg",
		],
	},
	{
		name: "Trao cho học sinh sinh viên khó khăn",
		images: [
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1727009872/IMG_5941_auxssw.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1727009854/IMG_5942_yjo1ww.jpg",
		],
	},
	{
		name: "Trao cho các em học sinh khó khăn huyện U Minh tỉnh Cà Mau",
		images: [
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1727009923/IMG_5940_y209vg.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1727009922/IMG_5938_jnen6t.jpg",
			"https://res.cloudinary.com/dbfh15hki/image/upload/v1727009922/IMG_5939_j7edgo.jpg",
		],
	},
];
